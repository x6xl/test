...
public class AccountManager {
    ...
    public CheckUsernameOrEmailVO checkUsernameOrEmail(CheckUsernameOrEmailDTO checkUsernameOrEmailDto) {
        ...
        if (!StringUtils.isEmpty(email)/*判断email是否为空*/) {
            email = email.trim();//去掉两端的空格等
            boolean isEmail = Validator.isEmail(email);//检查email字符串是否是邮件格式
            if (!isEmail) {
                rightEmail = false;
            } else {
                QueryWrapper<UserInfo> wrapper = new QueryWrapper<UserInfo>().eq("email", email);
                UserInfo user = userInfoEntityService.getOne(wrapper, false);//根据email查找
                if (user != null) {
                    rightEmail = true;
                } else {
                    rightEmail = false;
    ...
    public UserHomeVO getUserHomeInfo(String uid, String username) throws StatusFailException {
        ...
        List<String> disPlayIdList = new LinkedList<>();//从用户已解决的问题中获取问题的id和显示id
        ...
        if (pidList.size() > 0) {
            ...
            Map<Integer, List<UserHomeProblemVO>> map = problems.stream()
                    ...
                    .collect(Collectors.groupingBy(UserHomeProblemVO::getDifficulty));//把问题按照difficult进行分组
    ...
    private UserHomeProblemVO convertProblemVO(Problem problem) {//把一个Problem对象转换为一个UserHomeProblemVO对象
    ...
    public String userSignIn() throws StatusFailException{
        // 获取当前登录的用户
        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
        if (userRolesVo==null){
            throw new StatusFailException("你还未登入，请先登录后再来打卡");
        }
        String signInKey="signIn"+userRolesVo.getUid();
        redisUtils.set(signInKey,"yes");
        return "ok";
    }
    ...
    public String getUserSignInfo() {
        ...
        String signInKey = "signIn" + userRolesVo.getUid();
        String isSignIn = (String) redisUtils.get(signInKey);
        if ("yes".equals(isSignIn)) {
            return "yes";
        } else {
            return "no";
        }
