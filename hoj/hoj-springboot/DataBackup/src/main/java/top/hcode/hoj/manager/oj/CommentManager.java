...
public class CommentManager {
    ...
    public CommentListVO getComments(Long cid, Integer did, Integer limit, Integer currentPage) throws StatusForbiddenException, AccessException {
        ...
        if (cid == null && did != null) {
            ...
            if (discussion != null && discussion.getGid() != null) {
                accessValidator.validateAccess(HOJAccessEnum.GROUP_DISCUSSION);//验证是否有访问权限
        ...
        HashMap<Integer, Boolean> commentLikeMap = new HashMap<>();//储存用户对评论的点赞情况--哈希表，键值对

        if (userRolesVo != null) {
            // 如果是有登录 需要检查是否对评论有点赞
            List<Integer> commentIdList = new LinkedList<>();//LinkedList表示一个双向链表，可以存储任意类型的元素，提供快速的插入和删除操作
    ...
    public CommentVO addComment(Comment comment) throws StatusFailException, StatusForbiddenException, AccessException {
        ...
        comment.setFromAvatar(userRolesVo.getAvatar())
                ...
                .setFromUid(userRolesVo.getUid());//获取用户的信息到评论的对象中
        ...
        if (isOk) {
            //把comment对象中的属性复制到commentVo对象中的对应属性，表示把评论的信息转换为评论的视图信息
    ...
    @Transactional(rollbackFor = Exception.class)//表示事务，出现异常就回滚事务
