...
public class ContestScoreboardManager {
    ...
    public IPage getContestOutsideScoreboard(ContestRankDTO contestRankDto) throws StatusFailException, StatusForbiddenException {
        ...
        List<String> concernedList = contestRankDto.getConcernedList();//关注的用户列表
        Boolean removeStar = contestRankDto.getRemoveStar();//是否移除打星队伍
        Boolean forceRefresh = contestRankDto.getForceRefresh();//是否强制实时榜单
        ...
        boolean isContainsAfterContestJudge = Objects.equals(contest.getAllowEndSubmit(), true)
                && Objects.equals(contestRankDto.getContainsEnd(), true);//是否包含比赛结束后的提交记录
