...
public class DiscussionManager {
    ...
    public IPage<Discussion> getDiscussionList(Integer limit,
        ...
        if (categoryId != null) {//用户指定了分类id
            discussionQueryWrapper.eq("category_id", categoryId);
        }

        if (!StringUtils.isEmpty(keyword)) {

            final String key = keyword.trim();//keyword去除首尾空白
        ...
        if (!StringUtils.isEmpty(pid)) {
            discussionQueryWrapper.eq("pid", pid);//用户有没有指定题目id
        }

        if (!(admin && isAdmin)) {//显不显示管理员的讨论
        ...
        if (onlyMine && userRolesVo != null) {//只显示登录用户的评论
        ...
        if (!CollectionUtils.isEmpty(records)) {
            for (Discussion discussion : records) {
                if (userRolesVo == null) {
                    discussion.setContent(null);//用户没有登陆，隐藏评论
                } else if (!userRolesVo.getUid().equals(discussion.getUid())) {//评论是否是该用户的
    ...
    public List<Category> upsertDiscussionCategory(List<Category> categoryList) throws StatusFailException {
        List<Category> categories = categoryList.stream().filter(category -> category.getName() != null
                ...
                .collect(Collectors.toList());//去点name为空白的
