...
public class HomeManager {
    ...
    public List<HashMap<String, Object>> getHomeCarousel() {
        ...
        List<HashMap<String, Object>> apiList = fileList.stream().map(f -> {
            HashMap<String, Object> param = new HashMap<>(2);//初始容量为2
    ...
    public List<RecentUpdatedProblemVO> getRecentUpdatedProblemList() {
        ...
        problemQueryWrapper.orderByDesc("gmt_create");//按创建时间降序
        problemQueryWrapper.last("limit 10");//只查询10条
        List<Problem> problemList = problemEntityService.list(problemQueryWrapper);
        if (!CollectionUtils.isEmpty(problemList)){
            return problemList.stream()//把集合转换成流
                    .map(this::convertUpdatedProblemVO)//将一个流中的每个元素应用一个函数，得到一个新的流
                    .collect(Collectors.toList());//把流转化为集合
        }
    ...
    public SubmissionStatisticsVO getLastWeekSubmissionStatistics(Boolean forceRefresh) {
        SubmissionStatisticsVO submissionStatisticsVO = (SubmissionStatisticsVO) redisUtils.get(SUBMISSION_STATISTICS_KEY);//从redis缓存中获取一个SubmissionStatisticsVO对象，使用一个常量作为键

        boolean isRoot = SecurityUtils.getSubject().hasRole("root");
        forceRefresh = forceRefresh && isRoot;//如果参数为true且用户为root，那么强制刷新为true，否则为false
        ...
        if (submissionStatisticsVO == null || forceRefresh) {
            ...
            judgeQueryWrapper.apply("UNIX_TIMESTAMP(gmt_create) >= UNIX_TIMESTAMP('" + strTime + "')");//只查询创建时间大于等于strTime的记录
            ...
            redisUtils.set(SUBMISSION_STATISTICS_KEY, submissionStatisticsVO, 60 * 30);//存入redis缓存
        }
