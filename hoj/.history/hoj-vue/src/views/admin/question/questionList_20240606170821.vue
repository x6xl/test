    ...
    <el-row :gutter="18">
      <el-col :sm="24" :md="18" :lg="18">
        <el-card shadow>
          ...
          <el-table :data="questionList.question" stripe style="width: 100%"  @row-click="handleRowClick"> 
      ...
      <template slot-scope="props">  
        <div v-if="props.row.optionsLoaded">  
           
          <div v-for="option in props.row.options" :key="option.id">  
        ...
        <div v-else>  
          ...
          <el-button @click="loadOptions(props.row.questionId)">加载选项</el-button>  
        </div>  
      </template>  
    </el-table-column> --> 
...
import Question from './question.vue';
  export default {  
    ...
    searchQuestions() {  
      ...
      .then(response => {  
        this.questionList.question = response.data.data; 
      }).catch(error => {  
        console.error(error);  
      });  
    },  
   ...
   /*  admin_getOptions(questionId) {  
       api.admin_getOptions(questionId).then(response => {  
        this.questionList.options=response.data.data; 
      }).catch(error => {  
        console.error(error);  
      }); 
    },  */
      getQuestions() {  
        api.admin_getQuestionList().then(response => {  
          this.questionList.question = response.data.data;  
          console.log('66666:', this.questionList.question);
        })  
      },
