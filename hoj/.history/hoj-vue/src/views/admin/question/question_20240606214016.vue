...
export default {  
  ...
  created() {  
    // 更新options数组中的每个对象以包含questionId、author和authorId  
    this.questionForm.options.forEach(option => {  
      option.questionId = this.questionForm.questionId;  
      option.author = this.questionForm.author;  
      option.authorId = this.questionForm.authorId;  
    });  
  },  
  methods: {  
    addOption() {  
    ...
    submitForm(formName) {  
      ...
      this.$refs[formName].validate((valid) => {  
        if (valid) {  
            console.log('qid',this.questionForm.questionId); 
            console.log('author:',this.questionForm.author); 
            console.log('opc:',this.questionForm.options.optionContent);
            console.log('opauthor:',this.questionForm.options.author);
          // 假设你已经有了一个ajax函数，用于发送POST请求  
          api.admin_addQuestion(data)  
            .then((response) => {  
              // 处理响应，比如显示成功消息、清空表单等  
