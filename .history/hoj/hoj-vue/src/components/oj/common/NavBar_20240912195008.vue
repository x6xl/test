        ...
        >
          ...
          <template v-if="mode == 'defalut'">
            ...
            <el-submenu index="question">
              <template slot="title"
                ><i class="el-icon-circle-plus"></i>{{ $t('m.NavBar_Question') }}</template
              >
              <el-menu-item index="/Optquestion">{{
                $t('m.NavBar_OptQuestion')
              }}</el-menu-item>
      ...
      <mu-drawer :open.sync="opendrawer" :docked="false" :right="false">
        <mu-list toggle-nested>
          ...
          <mu-list-item
            ...
            :ripple="false"
            nested
            :open="openSideMenu === 'question'"
            @toggle-nested="openSideMenu = arguments[0] ? 'question' : ''"
          >
            <mu-list-item-action>
              <mu-icon value=":el-icon-circle-plus" size="24"></mu-icon>
            </mu-list-item-action>
            <mu-list-item-title>{{ $t('m.NavBar_Question') }}</mu-list-item-title>
            <mu-list-item-action>
              <mu-icon
                class="toggle-icon"
                size="24"
...
export default {
  ...
  data() {
    return {
      ...
      openmsgmenu: false,
      openSideMenu: '',
      imgUrl: require('D:\HP\图片\Saved Pictures\OjExam.png'),
      avatarStyle:
