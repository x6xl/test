    ...
    <el-card>
      <div slot="header">
        ...
        <div class="filter-row">
          ...
          <el-button
            v-if="isExamAddProblemRoute"
            type="primary"
            size="small"
            @click="submitExamProblems"
            icon="el-icon-check"
            >{{ $t('m.Add_Question_To_Exam')}}
          </el-button>
        </div>
      </div>
      <vxe-table
      ...
      >
        ...
        <vxe-table-column title="problemId" min-width="100" field="problemId">
        </vxe-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      examId: this.$route.params.examId, // 从路由中获取 examId
    };
  },

  computed: {
    isExamAddProblemRoute() {
      return this.$route.name === 'admin-exam-add-problem';
    }
  },
  mounted() {
    this.getProblems();
/*     if (this.isExamAddProblemRoute) {
...
/*     getSelectedProblems() {
  api.admin_ExamGetProblemId(this.examId).then((response) => {
    if (response.data.status === 200) {
      const existingProblemIds = response.data.data || [];
      this.selected_problems = this.problems.filter(problem => existingProblemIds.includes(problem.problemId));
      
      // 手动触发表格的勾选操作
      this.$nextTick(() => {
        this.setTableSelection(this.selected_problems);
      });
    }
  }).catch((error) => {
    console.error("获取考试代码题目失败", error);
  });
},

setTableSelection(selectedProblems) {
  const selectedIds = selectedProblems.map(problem => problem.id);
  this.$refs.xTable.setCheckboxRow(this.problems, false); // 先清空所有勾选
  this.$refs.xTable.setCheckboxRow(this.problems.filter(problem => selectedIds.includes(problem.id)), true); // 重新勾选
}, */
    ...
    submitExamProblems() {
      if (this.selected_problems.length <= 0) {
        myMessage.error(this.$i18n.t('m.No_Problems_Selected'));
        return;
      }

      const problemIds = this.selected_problems.map(problem => problem.problemId);
      const data = {
        examId: this.examId,
        problemId: problemIds
      };

      api.admin_examAddProblem(data).then((response) => {
        if (response.data.status === 200) {
