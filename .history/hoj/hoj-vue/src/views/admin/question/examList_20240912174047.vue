    ...
    getAndShowExamImageShots(examId) {
      api.getExamAllImageShot(examId)
        .then(response => {
          ...
          // 假设 examImageShots 的结构为 { "examJoinerId": [{ "id": "...", "url": "..." }, ...] }
          if (examImageShots) {
            Object.keys(examImageShots).forEach(examJoinerId => {
              if (examImageShots[examJoinerId].length > 0) {
                // 确保 examImages 和 examImageDialogVisible 初始化为对象
            ...
            this.$message.error('未找到考试截图');
          }
        })
        .catch(error => {
          console.error('获取考试截图失败', error);
          this.$message.error('获取考试截图失败');
        });
    },
