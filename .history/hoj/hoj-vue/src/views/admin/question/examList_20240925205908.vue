<template>
  ...
  <el-tooltip :content="$t('m.View_user_submission_records')" placement="top">
    <el-button type="info" icon="el-icon-view" @click.stop="viewExamRecords(scope.row.exam.examId)" size="mini"></el-button>
  </el-tooltip>
  <el-tooltip :content="$t('m.Get_Exam_process_ShotImage')" placement="top">
    <el-button type="success" icon="el-icon-picture" @click.stop="getAndShowExamImageShots(scope.row.exam.examId)" size="mini"></el-button>
      ...
      <el-input
      :placeholder="$t('m.Please_enter_userId_to_search')"
      v-model="keywords"
      ></el-input>
      <el-button type="primary" @click="searchExamRecord"icon="el-icon-search" circle></el-button>


      <el-select v-model="sort" @change="searchExamRecord" style="margin-bottom: 20px;">
       <el-option :label="$t('m.Desc')" :value=0></el-option>
       <el-option :label="$t('m.Asc')" :value=1></el-option>
      </el-select>
  ...
  export default {
    ...
    methods: {
      ...
      viewExamRecords(examid) {
        this.SelectExamId=examid;
      const examId = this.SelectExamId;
      const sort = this.sort;
      const keywords = this.keywords;
      this.SelectExamId=examId;
      api.getExamRecordList(examId, sort, keywords)
        .then(response => {
      ...
      searchExamRecord(){
        this.viewExamRecords(this.SelectExamId);
      },
