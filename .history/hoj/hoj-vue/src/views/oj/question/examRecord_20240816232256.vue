<template>
    <div>
      <h2>考试名称：{{ examName1 }}</h2>
      <p>考试描述：{{ examDescription1 }}</p>
      <p>考试时间：{{ examStartDate1 }} 至 {{ examEndDate1 }}</p>
      <p>考试得分: {{ examJoinScore }}</p>
        ...
        <el-row v-for="(question, index) in exam.selectQuestionVOs" :key="question.questionId">
          <el-col :span="24">
            ...
            <template v-if="answersMap && resultsMap && answersRightMap">
              <div>
                <p>正确答案: {{ answersRightMap[question.questionId].join(', ') }}</p>
                <p>用户答案: {{ answersMap[question.questionId] ? answersMap[question.questionId].join(', ') : '未选择' }}</p>
                <p :style="{ color: getTextColor(resultsMap[question.questionId]) }">结果: {{ resultsMap[question.questionId] === 'True' ? '正确' : '错误' }}</p>
              </div>
            </template>
