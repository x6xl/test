<template>  
    <div>  
      ...
      <el-form-item :label="$t('m.Opt_options') " v-if="questionType !== '4' && question.options.length">
      ...
      <el-form-item :label="$t('m.Opt_options')" v-if="(questionType === '2' && question.options.length) || (questionType !== '4' && question.options.length)">
  ...
  export default {  
    ...
    // 多选题，提交的是选中的所有选项内容
    submitResult = this.form.selectedAnswers.join(', ');
  } else {
    // 选择题，提交的是选项的内容
