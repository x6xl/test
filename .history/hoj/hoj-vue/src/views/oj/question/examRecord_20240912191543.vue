          ...
          <el-col :span="24">
            ...
            <template v-if="answersMap && resultsMap && answersRightMap">
              <div>
                ...
                <p>{{ $t('m.User_Answer')}}: {{ answersMap[question.questionId] ? answersMap[question.questionId].join(', ') : '未选择' }}</p>
                <p :style="{ color: getTextColor(resultsMap[question.questionId]) }">{{ $t('m.Result')}}: {{ resultsMap[question.questionId] === 'True' ? '正确' : '错误' }}</p>
              </div>
            </template>
