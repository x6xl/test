<template>
  <div>
    <h2 style="color:red">{{ $t('m.Exam_attentions')}}</h2>
    <h2 v-if="examStarted">{{ $t('m.Exam_name')}}：{{ examName1 }}</h2>
    <p v-if="examStarted">{{ $t('m.Exam_description')}}：{{ examDescription1 }}</p>
    <p v-if="examStarted">{{ $t('m.Exam_Time')}}：{{ examStartDate1 }} 至 {{ examEndDate1 }}</p>

    <el-input v-if="!examStarted" v-model="email" :placeholder="$t('m.Please_enter_your_email_address')"></el-input>
    <el-button v-if="!examStarted && !fetchingPassword" type="primary" :disabled="fetchingPassword"
      @click="sendPasswordByEmail">
      {{ fetchingPassword ? $t('m.Getting_it') : $t('m.Get_Exam_password') }}
    </el-button>
    <el-input v-if="!examStarted" v-model="password" :placeholder="$t('m.Please_enter_the_exam_password')" show-password></el-input>
    <el-button v-if="!examStarted" type="primary" @click="startExam">{{ $t('m.Start_Exam')}}</el-button>
      ...
      <el-row v-for="(question, index) in exam.selectQuestionVOs" :key="question.questionId">
        <el-col :span="24">
          ...
          <template v-else-if="question.questionContentVOs[0].questionType.toString() === '4'">
            <el-input v-model="examForm[question.questionId]" :placeholder="$t('m.Please_enter_your_answer')"></el-input>
          </template>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-button v-if="examStarted" type="primary" @click="submitExam">{{ $t('m.Submit_Exam')}}</el-button>

    <el-dialog :title="$t('m.Exam_Result')" :visible="showResultDialog" :close-on-click-modal="false" @close="closeResultDialog">
      <p>{{ $t('m.Your_Exam_Score')}}：{{ examScore }}，{{ $t('m.Exam_cost_Time')}}：{{ examTime }}{{ $t('m.minutes')}}</p>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="closeResultDialog">{{ $t('m.confirm')}}</el-button>
      </span>
    </el-dialog>
...
export default {
    ...
    submitExam() {
      ...
      api.finishExam(this.$route.params.examId, data)
        .then(response => {
          ...
          alert(this.$t('m.submit_exam_success'));
        ...
        .catch(error => {
          ...
          alert(this.$t('m.submit_exam_failed'));
          Notification.error({
            title: '错误',
            message: this.$message.error(this.$t('m.submit_exam_failed'))
          });
        });
      ...
      api.ExamSentPassword(data1)
        .then(response => {
          Notification.success({
            title: '成功',
            message: this.$message.error(this.$t('m.send_exam_password_success'))
          });
          this.fetchingPassword = false;
        })
        .catch(error => {
          console.error('获取考试密码失败', error);
          this.$message.error(this.$t('m.failed_to_get_exam_password'));
          this.fetchingPassword = false;
        });
    },
  ...
  beforeRouteLeave(to, from, next) {
    if (this.examStarted  && !this.examSubmitted && to.name !== 'examProblemDetail') {
      if (confirm(this.$t('m.Leave_the_exam_reminder'))) {
