  ...
  <div>
    <el-row :gutter="20">
      ...
      >
        ...
        <el-card :class="{ 'signed-in-background': isSignedIn }">
  <div slot="header" class="clearfix title">
    <span class="home-title panel-title">
      <i class="el-icon-thumb"></i>{{ $t('m.Check_in_information')}}
    </span>
  </div>

  <div class="card-content" style="text-align: center;">
    <p style="color: orange; font-size: 24px;">{{ currentDate }}</p>
    <p v-if="!isSignedIn" style="font-size: 18px;">{{ $t('m.Click_the_Punch_button')}}</p>
    <p v-else style="font-size: 18px;">{{ $t('m.Checked_in_today')}}</p>

    <el-button v-if="!isSignedIn" @click="signIn" style="margin-top: 10px;">{{ $t('m.Check_in')}}</el-button>
  </div>
</el-card>
        ...
        <el-card class="card-top">
          ...
          >
            <span class="home-title panel-title">
              <i class="el-icon-star-on"></i>{{ $t('m.Exam_Notification')}}
            </span>
          </div>
          <div v-for="notice in examNoticeList" :key="notice.examId" style="margin-bottom: 20px;">
            <p style="color: red;">{{ getNoticeType(notice) }}</p>
            <p>{{ $t('m.Exam_id')}}: {{ notice.examId }}</p>
            <p>{{ $t('m.Exam_Time')}}: {{ formatExamTime(notice.examStartDate, notice.examEndDate) }}</p>
          </div>
        </el-card>
...
export default {
  ...
  data() {
    return {
      currentDate: new Date().toLocaleDateString(),  // 示例：使用本地日期格式
      isSignedIn: "",
      examNoticeList: [],
      interval: 5000,
  ...
  methods: {
    ...
    api.getUserSignInfo()
      ...
      .catch(error => {
        console.error(this.$t('m.Failed_to_get_check_in_information'), error);
      });
  },
  signIn() {
    api.userSignIn()
      ...
      .catch(error => {
        console.error(this.$t('m.Failed_to_check_in'), error);
      });
  },


    getExamNotice() {
      // 假设api.getExamNotice()返回一个Promise，获取后端数据
      api.getExamNotice().then(response => {  
        // 处理后端返回的数据
        this.examNoticeList = response.data.data.map(item => ({
          type: Object.keys(item)[0], // 获取对象的键（add或update）
          ...item[Object.keys(item)[0]] // 获取对象的值（考试通知的详细信息）
        }));
      }).catch(error => {
        console.error(this.$t('m.Failed_to_get_exam_notification'), error);
      });
    },
    formatExamTime(startDate, endDate) {
      // 格式化考试时间，例如：2024-07-13 00:50:26 - 2024-07-19 08:00:00
      return `${startDate} - ${endDate}`;
    },
    getNoticeType(notice) {
      // 根据考试通知的属性判断是新增考试还是更新考试，并返回相应的文本
      if (notice.type === "add") {
        return this.$t('m.New_exams');
      } else {
        return this.$t('m.update_Exams');
      }
    },
